...
export const getConnectorConfig = async (baseUrl: string, token: string, connectorKey: string) => {
    ...
    const response = await fetch(
      `${baseUrl}/hub/connectors/${connectorKey}`,
      {
        method: "GET",
        headers,
      }
...
export const connectAccount = async (baseUrl: string, token: string, provider: string, credentials: Record<string, unknown>) => {
  const headers: Record<string, string> = {
    "Content-Type": "application/json",
  };
